...
| 22  | `react-native-svg` for icons, not characters or an icon font | Unicode arrows rendered as empty boxes on the author's TECNO device: the glyph is simply absent from the font that phone ships with, so a send button read as two broken characters. Nothing user facing may depend on an unknown font. An icon font would be a licensed binary with its own metrics to draw seven shapes; `react-native-svg` draws them as platform vectors, is autolinked, and needs no jest mock |
